...
const AuthComponent = React.memo(({ t, location, language: _language }) => {
  const redirect = React.useMemo(() => {
    const result =
      location.state?.from?.pathname || new URLSearchParams(location.search).get("redirect") || "/";
    return result;
  ...
  const handleForgotPasswordStateChange = React.useCallback(
    (isConfirmation) => {
      if (activeTab === "forgot-password") {
  ...
  return (
    <div
      className={cn("min-h-[100dvh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8")}
    >
...
export function Auth() {
  ...
  return (
    <AuthComponent
      t={propsRef.current.t}
      location={propsRef.current.location}
