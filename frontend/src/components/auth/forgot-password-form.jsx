  ...
  return (
    ...
    >
      <span className="inline-flex items-center justify-center gap-2 min-w-[120px]">
        {isSubmitting ? <Spinner className="size-4" /> : t('common.actions.submit', { defaultValue: 'Submit' })}
      </span>
    </Button>
  )
})
...
export const ForgotPasswordForm = ({ className, onStateChange, ...props }) => {
  ...
  const formSchema = useMemo(() => {
    return z.object({
      email: z
        .string()
        .min(1, t('pages.auth.forgotPassword.fields.email.validation.required', { defaultValue: 'Please enter your email' }))
        .refine((val) => {
          const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
          return emailRegex.test(val)
        }, {
          message: t('pages.auth.forgotPassword.fields.email.validation.invalid', { defaultValue: 'Please enter a valid email format' }),
        }),
    })
  }, [t])
      ...
      <div className={cn('space-y-6', className)} {...props}>
        ...
        >
          {isResending ? (
            ...
            <span>
              {t('pages.auth.forgotPasswordConfirmation.actions.resend', { defaultValue: 'Resend email' })}
            </span>
          )}
        </Button>
      </div>
    ...
    <Form {...form}>
      ...
      >
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t('pages.auth.forgotPassword.fields.email.label', { defaultValue: 'Email' })}</FormLabel>
              <FormControl>
                <Input 
                  type="email"
                  placeholder="" 
