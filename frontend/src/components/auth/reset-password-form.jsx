    ...
    >
      <span className="inline-flex items-center justify-center gap-2 min-w-[120px]">
        {isSubmitting ? (
          ...
          t("pages.auth.resetPassword.actions.submit", { defaultValue: "Reset Password" })
        )}
      </span>
    </Button>
  );
...
export const ResetPasswordForm = ({ className, token, ...props }) => {
  ...
  const validateResetToken = useMemo(
    () => authContext.validateResetToken,
    [authContext.validateResetToken]
  );
  const clearError = useMemo(() => authContext.clearError, [authContext.clearError]);
  const isResettingPasswordRef = useMemo(
    ...
    return z
      .object({
        password: z
          ...
          .min(
            1,
            t("pages.auth.resetPassword.fields.password.validation.required", {
              defaultValue: "Please enter your new password",
            })
          )
          .min(
            6,
            t("pages.auth.resetPassword.fields.password.validation.minLength", {
              defaultValue: "Password must be at least 6 characters",
            })
          ),
        confirm_password: z.string().min(
          1,
          t("pages.auth.resetPassword.fields.confirmPassword.validation.required", {
            defaultValue: "Please confirm your new password",
          })
        ),
      })
      .refine((data) => data.password === data.confirm_password, {
        message: t("pages.auth.resetPassword.fields.confirmPassword.validation.notMatch", {
      ...
      <div className={cn("space-y-6", className)}>
        <div className="text-center">
          <p className="text-muted-foreground text-sm">
            {t("pages.auth.resetPassword.messages.invalidToken", {
              defaultValue:
      ...
      <form className={cn("space-y-6", className)} onKeyDown={handleKeyDown} {...props}>
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                {t("pages.auth.resetPassword.fields.password.label", {
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>
                {t("pages.auth.resetPassword.fields.confirmPassword.label", {
