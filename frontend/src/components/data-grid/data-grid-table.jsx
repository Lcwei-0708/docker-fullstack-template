...
const DataGridTableBase = React.forwardRef(function DataGridTableBase({ children }, ref) {
...
const DataGridTableHeadRowCell = React.memo(
  function DataGridTableHeadRowCell({ children, header, dndRef, dndStyle }) {
    ...
    const isLastColumn =
      header.headerGroup.headers[header.headerGroup.headers.length - 1].id === header.id;
    ...
    if (isLastColumn) {
      ...
      const isNextColumnPinned =
        nextHeader.column.getIsPinned() !== false || nextHeader.column.id === "select";
      ...
      <th
        ...
        className={cn(
          ...
          props.tableLayout?.columnsResizable &&
            ...
            !(hasVerticalScrollbar && isLastColumn) &&
            "!border-e",
  ...
  (prev, next) => {
    ...
    if (prevIsResizing || nextIsResizing) {
      ...
      if (
        resizingColumnId &&
        prevColumn.id !== resizingColumnId &&
        nextColumn.id !== resizingColumnId
      ) {
        return true;
      }
    }
...
const DataGridTableHeadRowCellResize = React.memo(
  function DataGridTableHeadRowCellResize({ header }) {
    ...
    const resizeLinePosition = isPinnedEdge
      ? "before:right-0"
...
function DataGridTableBodyRowSkeletonCell({ children, column }) {
  ...
  return (
    ...
    >
      <div className="h-full flex items-center">{skeletonContent}</div>
    </td>
  );
}
...
const DataGridTableBodyRowCell = React.memo(
  function DataGridTableBodyRowCell({ children, cell, dndRef, dndStyle }) {
    ...
    if (hasVerticalScrollbar && isLastColumn) {
      ...
      const isNextColumnPinned =
        nextCell.column.getIsPinned() !== false || nextCell.column.id === "select";
      shouldHideRightBorder = isNextColumnPinned;
    }
    ...
    return (
      <td
        ...
        className={cn(
          ...
          hasVerticalScrollbar && isLastColumn
            ? "!border-e"
            : props.tableLayout?.cellBorder && !shouldHideRightBorder && "border-e",
          props.tableLayout?.columnsResizable && column.getCanResize() && "truncate",
          isResizing && "select-none [&_*]:pointer-events-none",
...
function DataGridTableRowSelect({ row, size, className }) {
  return (
    <>
      <div
        className={cn(
...
function DataGridTableRowSelectAll({ size, className }) {
