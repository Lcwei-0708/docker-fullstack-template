...
export function RolePermissionsMobile({
  ...
  const getCategoryLabel = (name) =>
    t(`pages.rolesManagement.categories.${toCamelKey(name)}`, name);
  const getAttributeLabel = (name) =>
    t(`pages.rolesManagement.attributes.${toCamelKey(name)}`, name);
                              ...
                              <div key={cat.name}>
                                ...
                                {hasAttrs ? (
                                  cat.attributes.map((attr, idx) => {
                                    ...
                                    const isLastRow =
                                      catIdx === g.categories.length - 1 &&
                                      idx === cat.attributes.length - 1;
