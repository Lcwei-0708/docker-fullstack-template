...
export const Layout = ({ children, showDock = true, dockPosition = "top-right" }) => {
    ...
    return (
      <SidebarProvider>
        ...
        <SidebarInset className={cn("h-svh overflow-hidden")}>
          ...
          <div className={cn("flex-1 overflow-auto min-h-0")}>{children}</div>
        </SidebarInset>
        <AlertDialog open={showLogoutDialog} onOpenChange={setShowLogoutDialog}>
          <AlertDialogContent>
            <AlertDialogHeader className={cn("flex flex-col", isMobile ? "gap-2" : "gap-0")}>
  ...
  return (
    <div className="h-[100dvh] max-h-[100dvh] relative">
      <div className={cn("h-full", isMobile && "overflow-y-auto")}>{children}</div>
      ...
      {showDock && (dockPosition === "top-left" || dockPosition === "bottom-left") && (
        <div className={cn("fixed z-50", dockPositionClasses[dockPosition])}>
          <Dock />
        </div>
      )}
    </div>
