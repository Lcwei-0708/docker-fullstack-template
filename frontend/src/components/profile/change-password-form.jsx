    ...
    return z
      .object({
        current_password: z
          .string()
          .min(1, {
            message: t("pages.profile.security.fields.currentPassword.validation.required"),
          })
          .min(6, {
            message: t("pages.profile.security.fields.currentPassword.validation.minLength"),
          }),
        new_password: z
          .string()
        ...
        confirm_new_password: z.string().min(1, {
          message: t("pages.profile.security.fields.confirmNewPassword.validation.required"),
        }),
      })
      ...
      try {
        ...
        if (error.isPasswordError) {
          const errorMessage = t(
            "pages.profile.security.fields.currentPassword.validation.incorrect"
          );
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5 md:space-y-2">
            <div className="flex justify-between text-sm text-foreground">
              <span className={strengthConfig.textColor}>{strengthConfig.label}</span>
            </div>
            <Progress
              value={passwordStrength}
              className={cn("h-2 w-full bg-muted", strengthConfig.barColor)}
            />
            <ul className="text-sm text-muted-foreground space-y-1 mt-2 md:mt-3">
              <li className="flex items-center gap-2">
                ...
                <span className={cn(form.watch("new_password")?.length >= 8 && "text-success")}>
              ...
              <li className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full flex-shrink-0 bg-muted-foreground" />
                <span className={cn(form.watch("new_password")?.match(/[A-Z]/) && "text-success")}>
              ...
              <li className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full flex-shrink-0 bg-muted-foreground" />
                <span className={cn(form.watch("new_password")?.match(/[0-9]/) && "text-success")}>
