...
@router.get(
    ...
    responses=parse_responses(
        {200: ("Successfully retrieved roles", RolesListResponse)}, common_responses
    ),
)
@require_permission([Permission.VIEW_ROLES, Permission.MANAGE_ROLES])
async def get_roles(
    request: Request, token: dict = Depends(verify_token), db: AsyncSession = Depends(get_db)
):
...
@router.post(
    ...
    responses=parse_responses(
        {200: ("Role created successfully", RoleResponse), 409: ("Role name already exists", None)},
        common_responses,
    ),
)
...
@router.get(
    ...
    responses=parse_responses(
        {
            200: (
                "Successfully retrieved role attributes mapping",
                RoleAttributesGroupedResponse,
...
@router.put(
    ...
    responses=parse_responses(
        {
            200: (
                "All role attributes processed successfully",
                RoleAttributeMappingBatchResponse,
                role_attributes_success_response_example,
            ),
            207: (
                "Role attributes processed with partial success",
                RoleAttributeMappingBatchResponse,
                role_attributes_partial_response_example,
            ),
            400: (
                "All role attributes failed to process",
                RoleAttributeMappingBatchResponse,
                role_attributes_failed_response_example,
            ),
        },
...
async def update_role_attribute_mapping_api(
    ...
    try:
        ...
        if batch_result.failed_count == 0:
            # All successful
            return APIResponse(
                code=200, message="All role attributes processed successfully", data=batch_result
            )
        elif batch_result.success_count == 0:
            # All failed - return 400 status code
            response = APIResponse(
                code=400, message="All role attributes failed to process", data=batch_result
            )
            return Response(
...
async def get_user_permissions_api(
    request: Request = None, token: dict = Depends(verify_token), db: AsyncSession = Depends(get_db)
):
