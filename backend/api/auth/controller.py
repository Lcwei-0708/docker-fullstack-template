...
from core.security import verify_email_verification_token, verify_password_reset_token, verify_token
...
async def token_api(
    request: Request, db: AsyncSession = Depends(get_db), redis_client=Depends(get_redis)
):
...
async def csrf_token_api(
    ...
    try:
        ...
        return APIResponse(
            code=200, message="CSRF token retrieved successfully", data=response_data
        )
    except AuthenticationException:
        raise HTTPException(status_code=401, detail="Invalid or expired session")
    except Exception:
...
async def reset_password_api(
    ...
    try:
        ...
        result = await reset_password(
            db, redis_client, token, request_data.new_password, client_ip, user_agent
        )
...
@router.get(
    ...
    responses=parse_responses({200: ("Token is valid", TokenValidationResponse)}, common_responses),
)
async def validate_reset_token_api(
    token: dict = Depends(verify_password_reset_token), db: AsyncSession = Depends(get_db)
):
...
async def forgot_password_api(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Please wait before requesting another password reset email"
        )
    except AuthenticationException:
...
async def get_password_reset_cooldown_api(
    ...
    try:
        ...
        response_data = PasswordResetCooldownResponse(cooldown_seconds=result["cooldown_seconds"])
...
async def resend_verification_api(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Please wait before requesting another verification email"
        )
    except AuthenticationException:
        raise HTTPException(status_code=403, detail="Account is disabled")
    except NotFoundException:
...
async def get_email_verification_cooldown_api(
    ...
    try:
        ...
        response_data = PasswordResetCooldownResponse(cooldown_seconds=remaining_seconds)
        return APIResponse(code=200, message="Cooldown status retrieved", data=response_data)
    except Exception:
        raise HTTPException(status_code=500)
