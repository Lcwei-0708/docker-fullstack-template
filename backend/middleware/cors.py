...
class CORSMiddleware(BaseHTTPMiddleware):
    ...
    def generate_cors_origins(self):
        ...
        for host in self.allowed_hosts:
            if host == "*":
                ...
                continue
            self.cors_origins.extend([f"http://{host}", f"https://{host}"])

    def is_whitelist_path(self, path: str) -> bool:
        """Check if path is in whitelist"""
        for whitelist_path in self.whitelist_paths:
            if path == whitelist_path or (
                whitelist_path.endswith("/") and path.startswith(whitelist_path)
            ):
                return True
        return False
    ...
    def _should_allow_origin(self, origin: str, is_whitelisted: bool, request: Request) -> bool:
        """Check if origin should be allowed"""
        if is_whitelisted:
            return bool(origin)
        return bool(
            origin
            and (self.is_allowed_origin(origin) or self._is_same_host_origin(origin, request))
        )
    ...
    def add_cors_headers(self, response, origin: str, is_whitelisted: bool, request: Request):
